...
function renderMultiToolbar(){
  ...
  if(count===1){
    ...
    els.multiToolbar.innerHTML=`<strong>${count} seleccionados</strong><span class="multi-divider"></span><button data-multi="left" data-tooltip="Alinear izquierda" aria-label="Alinear izquierda">⇤</button><button data-multi="center" data-tooltip="Centrar" aria-label="Centrar horizontalmente">↔</button><button data-multi="right" data-tooltip="Alinear derecha" aria-label="Alinear derecha">⇥</button><button data-multi="top" data-tooltip="Alinear arriba" aria-label="Alinear arriba">↥</button><button data-multi="middle" data-tooltip="Alinear centro vertical" aria-label="Centrar verticalmente">↕</button><button data-multi="bottom" data-tooltip="Alinear abajo" aria-label="Alinear abajo">↧</button><span class="multi-divider"></span><button data-multi="distribute-x" data-tooltip="Distribuir horizontalmente" aria-label="Distribuir horizontalmente">⋯</button><button data-multi="distribute-y" data-tooltip="Distribuir verticalmente" aria-label="Distribuir verticalmente">⋮</button><button data-multi="group" data-tooltip="Agrupar" aria-label="Agrupar">${uiIcon('component')}</button>`;
  }
...
function composeBackground(config){
  ...
  if(item.mode==='gradient')return item.gradientType==='radial'?`radial-gradient(circle at center, ${item.gradientStart}, ${item.gradientEnd})`:`linear-gradient(${item.gradientAngle}deg, ${item.gradientStart}, ${item.gradientEnd})`;
...
function generatedLayout(){
  return `---\nimport '../styles/global.css';\ninterface Props { title: string; description?: string; language?: string; ogImage?: string; noIndex?: boolean; }\nconst { title, description = '', language = 'es', ogImage = '', noIndex = false } = Astro.props;\n---\n<!doctype html>\n<html lang={language}>\n  <head>\n    <meta charset="UTF-8" />\n    <meta name="viewport" content="width=device-width" />\n    <meta name="description" content={description} />${googleFontsHeadMarkup()}\n    {noIndex && <meta name="robots" content="noindex,nofollow" />}\n    {ogImage && <meta property="og:image" content={ogImage} />}\n    <title>{title}</title>\n  </head>\n  <body>\n    <slot />\n  </body>\n</html>\n`;
}
...
function generatedPageAstro(page,componentNames){
  ...
  return `---\n${imports.join('\n')}\n---\n<BaseLayout title=${JSON.stringify(meta.title||page.name)} description=${JSON.stringify(meta.description||'')} language=${JSON.stringify(meta.language||'es')} ogImage=${JSON.stringify(meta.ogImage||'')} noIndex={${!!meta.noIndex}}>\n${body}${customScript}\n</BaseLayout>\n`;
}
...
function responsiveSiteHtml(){
  const previewCss=[generatedStyles().replace(/^@import[^;]+;\s*/gm,''),generatedTokensCss(),generatedGlobalClassesCss(),generatedElementsCss()].join('\n');
...
function codeStudioCss(){
  return [generatedStyles().replace(/^@import[^;]+;\s*/gm,''),generatedTokensCss(),generatedGlobalClassesCss(),generatedElementsCss()].join('\n\n');
}
...
function openCodeStudio(){
  ...
  $('#code-editor-html').value=codeStudioOriginal.html;
  $('#code-editor-css').value=codeStudioOriginal.css;
  $('#code-editor-js').value=codeStudioOriginal.js;
...
function resetCodeStudio(){
  $('#code-editor-html').value=codeStudioOriginal.html;$('#code-editor-css').value=codeStudioOriginal.css;$('#code-editor-js').value=codeStudioOriginal.js;setCodeStudioDirty(false);renderCodeStudioPreview();
}
