...
function normalizeSwiperConfig(value={}){
  ...
  return {
    ...
    speed:clamp(Number(source.speed)||defaults.speed,100,3000),autoplayDelay:clamp(Number(source.autoplayDelay)||defaults.autoplayDelay,800,20000),
    effect:['slide','fade'].includes(source.effect)?source.effect:'slide',direction:['horizontal','vertical'].includes(source.direction)?source.direction:'horizontal',
    loop:source.loop!==false,centered:source.centered===true,navigation:source.navigation!==false,pagination:source.pagination!==false,keyboard:source.keyboard!==false,autoplay:source.autoplay===true,pauseOnMouseEnter:source.pauseOnMouseEnter!==false
  };
}
function swiperRuntimeConfig(node){
  const config=normalizeSwiperConfig(node.swiper),fade=config.effect==='fade';
  const point=bp=>({slidesPerView:fade?1:config.slidesPerView[bp],spaceBetween:config.spaceBetween[bp]});
  return {
    slidesPerView:fade?1:config.slidesPerView.mobile,spaceBetween:config.spaceBetween.mobile,
    speed:config.speed,effect:config.effect,direction:config.direction,loop:config.loop,centeredSlides:config.centered,
    navigation:config.navigation,pagination:config.pagination?{clickable:true}:false,keyboard:config.keyboard?{enabled:true,onlyInViewport:true}:false,
...
function carouselInspector(node){
  const config=normalizeSwiperConfig(node.swiper),bp=BREAKPOINTS.includes(state.breakpoint)?state.breakpoint:'desktop';
  const slides=(node.children||[]).filter(child=>child.type==='slide').length;
  return `<div class="carousel-inspector">
    <header class="carousel-inspector-hero"><span>${uiIcon('gallery')}</span><div><small>SWIPER CAROUSEL</small><strong>${slides} ${slides===1?'slide':'slides'}</strong><em>${breakpointLabels[bp]} · ${state.canvasWidths[bp]} px</em></div><button type="button" data-carousel-preview title="Abrir Preview">${uiIcon('eye')}</button></header>
    <div class="carousel-responsive-grid">${field('Slides visibles',`<input type="number" min="1" max="8" step="0.25" value="${config.slidesPerView[bp]}" data-carousel-responsive="slidesPerView" data-carousel-breakpoint="${bp}">`)}${field('Espacio',`<div class="carousel-number-unit"><input type="number" min="0" max="120" step="1" value="${config.spaceBetween[bp]}" data-carousel-responsive="spaceBetween" data-carousel-breakpoint="${bp}"><span>px</span></div>`)}</div>
    <div class="field-grid">${field('Efecto',`<select data-carousel-field="effect"><option value="slide" ${config.effect==='slide'?'selected':''}>Slide</option><option value="fade" ${config.effect==='fade'?'selected':''}>Fade</option></select>`)}${field('Velocidad',`<div class="carousel-number-unit"><input type="number" min="100" max="3000" step="20" value="${config.speed}" data-carousel-field="speed"><span>ms</span></div>`)}</div>
    ${field('Dirección',`<div class="segmented two"><button type="button" data-carousel-value="horizontal" data-carousel-field-button="direction" class="${config.direction==='horizontal'?'active':''}">Horizontal</button><button type="button" data-carousel-value="vertical" data-carousel-field-button="direction" class="${config.direction==='vertical'?'active':''}">Vertical</button></div>`)}
    <div class="carousel-toggle-list">${carouselToggle('navigation','Flechas',config.navigation,'Anterior y siguiente')}${carouselToggle('pagination','Paginación',config.pagination,'Indicadores clicables')}${carouselToggle('loop','Loop infinito',config.loop,'Repite los slides')}${carouselToggle('centered','Centrar slide',config.centered,'Mantiene el activo al centro')}${carouselToggle('keyboard','Teclado',config.keyboard,'Navegación accesible')}</div>
    <div class="carousel-autoplay-card">${carouselToggle('autoplay','Autoplay',config.autoplay,'Desactivado por defecto por accesibilidad')}<div class="carousel-autoplay-options ${config.autoplay?'':'is-disabled'}">${field('Intervalo',`<div class="carousel-number-unit"><input type="number" min="800" max="20000" step="100" value="${config.autoplayDelay}" data-carousel-field="autoplayDelay" ${config.autoplay?'':'disabled'}><span>ms</span></div>`)}${carouselToggle('pauseOnMouseEnter','Pausar al hover',config.pauseOnMouseEnter)}</div></div>
    <div class="carousel-slide-actions"><button type="button" data-carousel-add-slide>${uiIcon('plus')} Añadir slide</button><small>Selecciona cada slide desde Capas para editar su contenido.</small></div>
  </div>`;
}
