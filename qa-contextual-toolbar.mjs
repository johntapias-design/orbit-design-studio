...
try {
  ...
  await setViewport(1600,900);
  const swiperSuite=await evaluate(`(()=>{
    ...
    const standalone=qa.generatedAstro();const files=qa.projectFiles();const packageFile=JSON.parse(files.find(file=>file.name==='package.json').data);const pageFile=files.find(file=>file.name==='src/pages/index.astro')?.data||'';
    return {imported:report.nodes>=9,staticDesigner:!!designer&&!document.querySelector('#canvas-page swiper-container'),inspector:!!inspector,responsiveControls:inspector?.querySelectorAll('[data-carousel-responsive]').length===2,toggles:(inspector?.querySelectorAll('[data-carousel-field]').length||0)>=8,slideAdded:document.querySelectorAll('.canvas-slide').length===beforeSlides+1,standaloneElement:standalone.includes('<swiper-container')&&standalone.includes('<swiper-slide'),standaloneRuntime:standalone.includes('swiper@14.0.6')&&standalone.includes('initialize()'),conditionalDependency:packageFile.dependencies.swiper==='^14.0.6',astroRuntime:pageFile.includes("swiper/element/bundle")&&pageFile.includes('data-orbit-swiper'),responsiveConfig:standalone.includes('breakpoints')};
  })()`);
