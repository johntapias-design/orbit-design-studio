...
try {
  ...
  const orbitJsonStudioUi=await evaluate(`(()=>{
    ...
    const frame=document.getElementById('orbit-json-preview');const result={opened:document.getElementById('modal-title')?.textContent==='Importar a Orbit',contractUi,blocked,paths,noMutation,preview:!!frame&&frame.getAttribute('sandbox')===''&&frame.srcdoc.includes('Preview'),readiness:!!document.querySelector('.orbit-json-readiness'),download:!!document.querySelector('[data-download-orbit-v13]'),commit:!!document.querySelector('[data-commit-orbit-import]')};document.querySelector('[data-close-modal]')?.click();return result;
  })()`);
