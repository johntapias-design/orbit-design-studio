...
try {
  ...
  const orbitJsonStudioUi=await evaluate(`(()=>{
    ...
    const frame=document.getElementById('orbit-json-preview'),repairReport=document.querySelector('.orbit-json-repair-report');const result={opened:document.getElementById('modal-title')?.textContent==='Importar a Orbit',contractUi,blocked,paths,noMutation,preview:!!frame&&frame.getAttribute('sandbox')===''&&frame.srcdoc.includes('Preview'),readiness:!!document.querySelector('.orbit-json-readiness'),repairReport:!!repairReport,repairExplanation:repairReport?.textContent.includes('Se reparó el ID del nodo')&&repairReport?.textContent.includes('Se convirtió el tipo'),repairPaths:repairReport?.textContent.includes('$.nodes[1].id'),repairPending:repairReport?.textContent.includes('Tu proyecto todavía no ha cambiado'),download:!!document.querySelector('[data-download-orbit-v13]'),commit:!!document.querySelector('[data-commit-orbit-import]')};document.querySelector('[data-close-modal]')?.click();return result;
  })()`);
