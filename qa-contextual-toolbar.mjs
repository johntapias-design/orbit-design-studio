...
try {
  ...
  const reusableComponentSuite=await evaluate(`(async()=>{
    const qa=window.__ORBIT_QA__,image='data:image/webp;base64,AAAA';
    qa.loadOrbitDocument({version:13,projectName:'Cliente A',pageMeta:{language:'es',title:'Cliente A',description:''},tokens:{colors:{brand:{name:'Brand',value:'#ef5a24',cssVar:'--color-brand'}},typography:{},spacing:{},radius:{},shadows:{}},assets:[{id:'asset-logo',name:'logo.webp',type:'image/webp',src:image,alt:'Logo'}],components:[],globalClasses:[{id:'class-nav',name:'nav-client-a',styles:{base:{display:'flex',gap:'20px',color:'var(--color-brand)'}},states:{}}],nodes:[{id:'client-header',type:'section',name:'Header Cliente A',htmlTag:'header',globalClassIds:['class-nav'],styleClassId:'class-nav',styleEditMode:'shared',styles:{base:{}},children:[{id:'client-logo',type:'image',name:'Logo',src:image,alt:'Logo',styles:{base:{width:'120px'}}},{id:'client-cta',type:'button',name:'CTA',content:'Contacto',styles:{base:{background:'var(--color-brand)'}}}]}]},'client-header');
    document.querySelector('[data-tab="components"]')?.click();document.querySelector('[data-component-library-view="shared"]')?.click();await new Promise(resolve=>setTimeout(resolve,180));const sharedView=!!document.querySelector('.component-shared-toolbar');document.querySelector('[data-save-reusable-component]')?.click();const form=document.querySelector('[data-reusable-component-form]'),modal=!!form;if(form){form.elements.name.value='Header reutilizable QA';form.elements.category.value='headers';form.requestSubmit();}await new Promise(resolve=>setTimeout(resolve,220));const savedCard=[...document.querySelectorAll('.component-shared-card')].find(card=>card.textContent.includes('Header reutilizable QA')),saved=!!savedCard&&savedCard.textContent.includes('Cliente A');
    qa.loadOrbitDocument({version:13,projectName:'Cliente B',pageMeta:{language:'es',title:'Cliente B',description:''},tokens:{colors:{brand:{name:'Brand B',value:'#0055ff',cssVar:'--color-brand'}},typography:{},spacing:{},radius:{},shadows:{}},assets:[],components:[],globalClasses:[],nodes:[{id:'target',type:'section',name:'Target',styles:{base:{}}}]},'target');document.querySelector('[data-tab="components"]')?.click();document.querySelector('[data-component-library-view="shared"]')?.click();await new Promise(resolve=>setTimeout(resolve,80));const insert=[...document.querySelectorAll('[data-insert-reusable-component]')].find(button=>button.closest('.component-shared-card')?.textContent.includes('Header reutilizable QA'));insert?.click();await new Promise(resolve=>setTimeout(resolve,80));const snapshot=qa.workspaceSnapshot(),inserted=snapshot.nodes.find(node=>node.name==='Header reutilizable QA'),component=snapshot.components.find(item=>item.name==='Header reutilizable QA'),colorKeys=Object.keys(snapshot.tokens.colors||{});const dependencies=!!inserted&&snapshot.globalClasses.some(item=>item.name==='nav-client-a')&&snapshot.assets.some(item=>item.src===image)&&colorKeys.some(key=>key!=='brand'&&key.includes('brand'));
    document.querySelector('[data-tab="components"]')?.click();document.querySelector('[data-component-library-view="shared"]')?.click();const remove=[...document.querySelectorAll('[data-delete-reusable-component]')].find(button=>button.closest('.component-shared-card')?.textContent.includes('Header reutilizable QA')),originalConfirm=window.confirm;if(remove){window.confirm=()=>true;remove.click();await new Promise(resolve=>setTimeout(resolve,120));window.confirm=originalConfirm;}const libraryRemoved=![...document.querySelectorAll('.component-shared-card')].some(card=>card.textContent.includes('Header reutilizable QA'));
    return {sharedView,modal,saved,inserted:!!inserted,componentPro:!!component&&component.masterId===inserted?.id,dependencies,idsRegenerated:inserted?.id!=='client-header'&&inserted?.children?.[0]?.id!=='client-logo',sourceIndependent:inserted?.componentSource==='master',safeDelete:!!inserted&&libraryRemoved};
  })()`);
  ...
  for (const [key,value] of Object.entries(reusableComponentSuite)) if(!value) failures.push(`reusable-components:${key}`);
